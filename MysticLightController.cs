...
public class MysticLightController : IDisposable
{
    ...
    public bool Initialize()
    {
        ...
        {
            Console.WriteLine("MysticLight_SDK.dll wasn't found. Place it next to ChromaSync.exe " +
        ...
        if (status != 0)
        {
            Console.WriteLine($"Mystic Light SDK failed to initialize (code {status}). " +
    ...
    private void LoadDevices()
    {
        ...
        {
            Console.WriteLine($"Could not enumerate Mystic Light devices (code {status}).");
            return;
        }
        ...
        for (int i = 0; i < types.Length; i++)
        {
            ...
            Console.WriteLine($"[MysticLight-Diag] Device[{i}]: Type = '{type}', Reported LedCount = '{reportedCount}'");

            string[] ledNames = Array.Empty<string>();

            int ledStatus = MysticLightNative.GetLedName(type, out var retrievedNames);
            Console.WriteLine($"[MysticLight-Diag] MLAPI_GetLedName('{type}') -> Status Code: {ledStatus}");

            if (retrievedNames is null)
            {
                Console.WriteLine($"[MysticLight-Diag] MLAPI_GetLedName('{type}') -> retrievedNames array is null");
            }
            else
            {
                Console.WriteLine($"[MysticLight-Diag] MLAPI_GetLedName('{type}') -> retrievedNames array length: {retrievedNames.Length}");
                for (int j = 0; j < retrievedNames.Length; j++)
                {
                    Console.WriteLine($"[MysticLight-Diag] MLAPI_GetLedName('{type}') -> raw[{j}] = \"{retrievedNames[j]}\"");
                }
            }
            ...
            if (ledStatus == 0 && retrievedNames is not null)
            {
                ...
                for (int j = 0; j < ledNames.Length; j++)
                {
                    ...
                    {
                        Console.WriteLine($"[MysticLight-Diag] Device: '{type}' -> Entry [{j}] is null or empty string");
                        continue;
                    }
                    ...
                    try
                    {
                        ...
                        if (parts.Length >= 2)
                        {
                            var areaIndex = parts[0];
                            var fullName = parts[1];
                            Console.WriteLine($"[MysticLight] Device: '{type}' -> AreaIndex: {areaIndex}, FullName: '{fullName}'");
                        }
                        else
                        {
                            Console.WriteLine($"[MysticLight-Diag] Device: '{type}' -> Unexpected format (Split(':') produced {parts.Length} parts) for entry [{j}]: \"{name}\"");
                            Console.WriteLine($"[MysticLight] Device: '{type}' -> Found LED Name: '{name}'");
                        }
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"[MysticLight-Diag] Device: '{type}' -> Exception parsing entry [{j}] \"{name}\": {ex}");
                    }
                }
            }
            else
            {
                Console.WriteLine($"Could not retrieve LED names for device {type} (code {ledStatus}).");
            }
    ...
    public void SaveInitialColor(string type)
    {
        ...
        if (status == 0)
        {
            ...
            Console.WriteLine($"[MysticLight] Saved initial color for '{type}': RGB({r},{g},{b})");
        }
        else
        {
            Console.WriteLine($"[MysticLight] Could not get initial color for '{type}' (code {status}). Defaulting restore to RGB(0,0,0).");
    ...
    public void SetLedStyle(string type, int index, string style)
    {
        ...
        Console.WriteLine($"[MysticLight] SetLedStyle(Type: '{type}', Index: {index}, Style: '{style}') -> Status Code: {status}");
    }
    ...
    public void SetColor(IEnumerable<string> deviceTypes, Color color)
    {
        ...
        foreach (var type in deviceTypes)
        {
            ...
            if (device.LedNames.Length > 0)
            {
                ...
                Console.WriteLine($"[MysticLight] SetLedColorsSync [by-name] (Type: '{type}', LEDs: {ledNames.Length}, RGB: {color.R},{color.G},{color.B}) -> Status Code: {status}");
            }
            else
            {
                int status = MysticLightNative.SetLedColor(type, 0, color.R, color.G, color.B);
                Console.WriteLine($"[MysticLight] SetLedColor [indexed] (Type: '{type}', Index: 0, RGB: {color.R},{color.G},{color.B}) -> Status Code: {status}");
            }
        }
    }
    ...
    public void RestoreOriginalState()
    {
        lock (_shutdownLock)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"Restoring original color: RGB({r},{g},{b})...");
                    int status = MysticLightNative.SetLedColor(type, 0, r, g, b);
                    Console.WriteLine($"[MysticLight] Restored color for '{type}' -> Status Code: {status}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MysticLight] Error restoring original color: {ex.Message}");
            }
            finally
            {
                Release();
    ...
    public void Release()
    {
        ...
        try
        {
            ...
            Console.WriteLine($"[MysticLight] Mystic Light SDK released (code {status}).");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[MysticLight] Error releasing Mystic Light SDK: {ex.Message}");
        }
        finally
        {
            _initialized = false;
